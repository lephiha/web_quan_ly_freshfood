...
import * as userService from 'src/services/productService';
import type { ProductProps } from '../sections/product/product-table-row';
...
export const _statistic = [...Array(24)].map((_, index) => {
...
export const _products = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      const resultListProduct = await userService.getProduct();
...
export const _products_example = [Array].map((_, index) => {
  const setIndex = index + 1;
